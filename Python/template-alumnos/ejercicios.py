...
def esta_ordenado_asc(v):
    i = 0
    longitud = len(v)
    ### esMenor = False ==> Provocaba que nunca entre al while y jamas verificaba que se cumpla
    esMenor = True 
...
def esta_ordenado(v):
    ...
    if longitud == 0 or longitud == 1:
        ...
        ### return esta_ordenado_desc(v) and esta_ordenado_asc(v) ==> Ilogico
...
def es_primo(numero:int):
    if numero > 2:
        i = 2
        divide = False
        ### while i <= numero and not divide ==> Pero i no debe ser igual a numero ya que su division es resto 0
        while i < numero and not divide: 
            ### divide:bool = numero % i != 0 ==> Pero justamente si es != es porque no lo divide
...
def pertenece(elemento:int, v:list[int]):
    ...
    if longitud == 0:
        ...
        while i < longitud and sigo:
            sigo = v[i] != elemento
            ### i += 1 ==> Esto provocaba que si el elemento estaba en la ultima posicion, igual se sumaba i, y quedaba longitud igual a i
            if sigo: i += 1 
        return i < longitud
...
def desvio_estandar(v: list[float]):
    def suma_de_cuadrados(v):
      ...
      for i in range(len(v)):
          ### suma_de_cuadrados += suma_de_cuadrados + (v[i] - promedio(v)) ** 2 ==> Pero el operador += ya esta incluyendo suma_de_cuadrados +
          suma_de_cuadrados += (v[i] - promedio(v)) ** 2 
...
def maximo_comun_divisor(x, y):
  ...
  ### Basicamente estaba mal implementado el algoritmo de euclides
...
def valor_medio():
    ...
    for val in miArchivo.read().split(): ### El archivo tenia mal escrito un numero. El numero: 0.875973, estaba de la forma: 0875973. Lo cual rompia todo.
        acum += float(val)
        cont += 1
    miArchivo.close()
    return acum / cont
...
def cant_palabras(filename):
    ...
    # En este caso, se usa el método split() para separar las palabras, pero este
    # método no pueden usarlo en sus implementaciones (como en el TP). Deben implementar su propia
    # función para separar las palabras.
    for palabra in miArchivo.read().split(): ### El archivo tenia mal escrito una parte. Estaba contando "operar,y" como una unica palabra, le faltaba el espacio entre la coma y la 'y'
        cont += 1
    miArchivo.close()
    return cont
